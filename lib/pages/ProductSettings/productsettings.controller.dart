...
class ProductSettingsPageController {
  ...
  ProductSettingsPageController({this.product}) {
    ...
    ingredientsChipWidget =
        IngredientsChips(selectedIngredients: selectedIngredients);
    ...
    for (ProductStatus value in ProductStatus.values) {
      ...
      switch (value) {
        case ProductStatus.Archived:
          valueFriendlyName = "Archiviert";
        case ProductStatus.Available:
          valueFriendlyName = "Verfügbar";
        case ProductStatus.SoldOut:
          valueFriendlyName = "Ausverkauft";
        default:
          valueFriendlyName = "Keinen";
      }
  ...
  Future<APIResponse<List<BPCategory>?>> getCategories() =>
      APIService.getCategories();

  Future<APIResponse<List<BPIngredient>?>> getIngredients() =>
      APIService.getIngredients();

  Widget generateCategoryList(
      BuildContext context, AsyncSnapshot<List<BPCategory>> snapshot) {
    ...
    return DropdownButtonFormField(
      ...
      decoration: const InputDecoration(
          border: OutlineInputBorder(), labelText: 'Kategorie'),
    );
  }
  ...
  void onSave(BuildContext context) async {
    ...
    if (product == null) {
      response = await APIService.addProduct(
          ...
          price: double.parse((priceTIController.text)
              .substring(0, priceTIController.text.length - 1)
              .replaceAll(",", ".")),
      ...
      response = await APIService.updateProduct(
          ...
          status: selectedStatus,
          description: descriptionTIController.text,
          ingredients: selectedIngredients.keys
              .where((element) => selectedIngredients[element] == true)
              .toList(),
          extras: selectedExtras.keys
              .where((element) => selectedExtras[element] == true)
              .toList());
    }
