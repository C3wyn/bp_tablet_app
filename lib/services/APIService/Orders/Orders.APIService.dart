...
class OrdersAPIService {
  ...
  Future<APIResponse> addOrder(BPOrder order) async {
    ...
    var response = await http.post(
        Uri.parse('http://${BPEnvironment.BASEURL}/createOrder'),
        headers: headers,
        body: body);
    Map<String, dynamic> data = jsonDecode(response.body);
    print(data);
    print(response.statusCode);
    if (data['acknowledged'] == true && response.statusCode == 200) {
      CartService.currentOrder =
          BPOrder(status: OrderStatus.Created, orderItems: []);
      return APIResponse(
          200, "Successfull", "Produkt erfolgreich hinzugefügt", null);
    }
  ...
  String _orderToJson(BPOrder order) {
    ...
    for (BPOrderItem item in order.orderItems) {
      var ingredients = [];
      for (BPIngredient ingredient in item.selectedIngredients.keys
          .where((x) => !item.selectedIngredients[x]!)) {
        ingredients.add({
      ...
      var extras = [];
      for (BPExtra extra
          in item.selectedExtras.keys.where((x) => item.selectedExtras[x]!)) {
    ...
    return jsonEncode({
      ...
      "deliveryType": order.deliveryType == OrderDeliveryType.EatHere
          ? "Eat Here"
          : "Take Away",
